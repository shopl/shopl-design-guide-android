package com.shopl.sdg.template.popup.center.extension
...
import com.shopl.sdg.template.popup.center.SDGCenterPopupButton
 ...
 *
 * 정보 전달 및 컨펌을 위한 콘텐츠를 포함하는
 * 화면 중앙에 노출되는 팝업 컴포넌트입니다.
 *
 * 콘텐츠 영역은 LazyColumn 기반으로 구성되어,
 * 길이에 따라 스크롤되는 리스트 형태의 UI를 지원합니다.
 *
    ...
    Dialog(
        onDismissRequest = {
            when (buttonOption) {
                ...
                is SDGCenterPopupButtonOption.TwoOption -> buttonOption.run {
                    onDismiss?.let { it() } ?: onClickCancel()
                }

                is SDGCenterPopupButtonOption.DeleteOption -> buttonOption.run {
                    onDismiss?.let { it() } ?: onClickCancel()
                }
            }
        },
